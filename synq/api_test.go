...
func TestMakeReq(t *testing.T) {
	...
	assert.Nil(err)
	assert.NotNil(req)
	assert.Equal("/"+SYNQ_LEGACY_VERSION+"/video/create", req.URL.Path)
...
func TestHandlePostFail(t *testing.T) {
	...
	assert.NotNil(err)
	assert.Equal("parse :://noprotocol.com/"+SYNQ_LEGACY_VERSION+"/video/path: missing protocol scheme", err.Error())
}
...
func TestHandlePostSuccess(t *testing.T) {
	...
	r := reqs[0]
	v := vals[0]
	assert.Equal("/"+SYNQ_LEGACY_VERSION+"/video/create", r.RequestURI)
